...
function createMocks(opts?: { healMinIntervalMs?: number; readDef?: object; editDef?: object }) {
  ...
  const stop = createSelfHealWatcher({
    ...
    rootCtx,
    agent,
    agentCtx: agentCtx as unknown as {
...
describe("self-heal watcher", () => {
  it("restores edit after external takeover and logs warn with agent id", async () => {
    ...
    expect(
      warnCalls.some(
        (m) =>
          m.includes("agent-1") && m.includes("Rianico/dsh-better-edit#43") && m.includes("(1/2)"),
      ),
    ).toBe(true);
  ...
  it("restores read defensively and includes agent id", async () => {
    ...
    expect(
      warnCalls.some((m) => m.includes("restored hash-anchored read") && m.includes("agent-1")),
    ).toBe(true);
  });
  ...
  it("throttle caps at 2 restores then error and stops healing", async () => {
    const { toolMap, editDef, fakeDef, getChangeHandler, warnCalls, errorCalls } = createMocks({
      healMinIntervalMs: 0,
    });
    const handler = getChangeHandler()!;
    // first takeover
    ...
    expect(
      errorCalls.some(
        (m) =>
          m.includes("self-heal disabled") &&
          m.includes("agent-1") &&
          m.includes("Rianico/dsh-better-edit#43"),
      ),
    ).toBe(true);
  ...
  it("degrades gracefully when layers throws — logs warn and does not crash", async () => {
    ...
    handler!();
    await flushMicrotasks();
    expect(warnCalls.some((m) => m.includes("self-heal failed") && m.includes("agent-2"))).toBe(
      true,
    );
  });
