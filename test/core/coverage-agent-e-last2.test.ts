...
describe("coverage-agent-e last2", () => {
  it("covers undo-edit and tool-read", async () => {
    await withTempFile("a.txt", "a\nb\nc\n", async ({ cwd }) => {
      ...
      const r1 = await harness.readTool.execute("read", {
        path: "a.txt",
        offset: 1,
        limit: 1,
      } as any);
      ...
      const u1 = await harness
        .getTool("undo_last_edit")
        .execute("undo_last_edit", { path: "a.txt" } as any);
  ...
  it("covers edit with valid hash", async () => {
    await withTempFile("a.txt", "a\nb\nc\n", async ({ cwd }) => {
      ...
      const hash = getText(readRes)
        ...
        .find((l) => l.includes("│a"))!
        .split("│")[0]!;
      const editRes = await harness.editTool.execute("edit", {
        path: "a.txt",
        edits: [[hash, hash, "AA"]],
      } as any);
      expect(getText(editRes).length).toBeGreaterThan(0);
      const undoRes = await harness
        .getTool("undo_last_edit")
        .execute("undo_last_edit", { path: "a.txt" } as any);
