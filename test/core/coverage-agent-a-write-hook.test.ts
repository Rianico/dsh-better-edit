...
describe("write-hook coverage agent-a", () => {
  ...
  describe("servedHashEchoDenial", () => {
    it("returns undefined when no echo", async () => {
      ...
      try {
        ...
        const denial = await withWorkspace(cwd, () =>
          servedHashEchoDenial(io, file, "clean content\n", cwd, sessionKey),
        );
    ...
    it("aborts if signal aborted", async () => {
      ...
      await expect(
        withWorkspace("/tmp", () =>
          servedHashEchoDenial(io, "/tmp/x", "content", "/tmp", "k", ctrl.signal),
        ),
      ).rejects.toThrow();
    });
    ...
    it("aborts before served load if signal aborted after resolve", async () => {
      ...
      try {
        ...
        const res = await withWorkspace(dir, () =>
          servedHashEchoDenial(io2, file, "hi\n", dir, "k2"),
        );
  ...
  describe("registerWriteHook", () => {
    it("denies when echo, allows clean, fails open on error, handles non-write", async () => {
      ...
      try {
        ...
        const dec1 = await pre(
          {
            name: "read",
            arguments: { path: file },
            signal: new AbortController().signal,
            agent: { id: sessionKey, session: { id: sessionKey, header: { cwd: dir } } },
          } as any,
          nextPre,
        );
        ...
        const dec2 = await pre(
          {
            name: "write",
            arguments: { file_path: file, content: "clean\n" },
            signal: new AbortController().signal,
            agent: { id: sessionKey, session: { id: sessionKey, header: { cwd: dir } } },
          } as any,
          nextPre2,
        ...
        const dec3 = await pre(
          {
            name: "write",
            arguments: { file_path: file, content: preview.text },
            signal: new AbortController().signal,
            agent: { id: sessionKey, session: { id: sessionKey, header: { cwd: dir } } },
          } as any,
          nextPre3,
        ...
        const dec4 = await pre(
          {
            name: "write",
            arguments: { file_path: 123 } as any,
            signal: new AbortController().signal,
            agent: { id: sessionKey, session: { id: sessionKey, header: { cwd: dir } } },
          } as any,
          nextPre4,
        ...
        const nextPost = vi.fn(
          async () =>
            ({ kind: "accept", content: [{ type: "text", text: "orig" }] }) as PostToolDecision,
        );
        const res1 = await post(
          { name: "read", arguments: {} } as any,
        ...
        const res2 = await post(
          { name: "write", arguments: { file_path: file } } as any,
          { isError: true } as any,
          nextPost2,
        );
        ...
        const nextPost4 = vi.fn(
          async () =>
            ({ kind: "accept", content: [{ type: "text", text: "write ok" }] }) as PostToolDecision,
        );
        ...
        const nextPost6 = vi.fn(
          async () =>
            ({ kind: "accept", content: [{ type: "text", text: "ok" }] }) as PostToolDecision,
        );
        const res6 = await postFail(
          {
            name: "write",
            arguments: { file_path: file },
            signal: new AbortController().signal,
