...
describe("coverage-agent-d canonical-path", () => {
  ...
  it("canonicalAsync ELOOP", async () => {
    ...
    let err2: any;
    try {
      await canonicalAsync(a);
    } catch (e) {
      err2 = e;
    }
    expect(err2?.code).toBe("ELOOP");
    await rm(tmp, { recursive: true, force: true });
  });
});
...
describe("coverage-agent-d session-view", () => {
  ...
  it("_mergeServedRows branches", () => {
    ...
    // truncate
    expect(_mergeServedRows(cur, [{ position: 0, hash: "ccc" }], { truncateTo: 2 })).toEqual([
      "ccc",
      "bbb",
    ]);
    // clearFrom
  ...
  it("computeDrift branches", () => {
    ...
    expect(
      computeDrift({
        ...
        resultHashes: ["aaa"],
        resultLines: ["line"],
        range: { startHash: "aaa", endHash: "aaa", startLine: 1, endLine: 1, delta: 0 } as any,
        reported: new Set(),
      }),
    ).toBeUndefined();
    ...
    const r2 = computeDrift({
      ...
      resultHashes: hashes,
      resultLines: lines,
      range: { startHash: "h0", endHash: "h1", startLine: 2, endLine: 3, delta: 0 } as any,
      reported: new Set(),
      cap: 10,
    });
