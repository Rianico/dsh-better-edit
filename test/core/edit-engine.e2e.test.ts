...
import { withTempFile, withTempBytes, setupIntegrationTest, getText } from "../support/fixtures.js";
import { ctxFsIO, type FileIO } from "../../src/fs-bridge.js";

type Tool = {
  execute: (_callId: string, params: unknown) => Promise<{ content: Array<{ text?: string }> }>;
};

function batchTool(harness: ReturnType<typeof setupIntegrationTest>): Tool {
  return harness.getTool("batch_edit") as unknown as Tool;
}

function undoTool(harness: ReturnType<typeof setupIntegrationTest>): Tool {
...
function bomStrippingCtxIO(): FileIO {
  const fs = {
    ...
    readText: async (target: { displayPath: string }) =>
      new TextDecoder("utf-8", { fatal: true }).decode(await readFile(target.displayPath)),
    readBytes: async (
...
describe("edit-sequence engine — end-to-end through the tool builders", () => {
  ...
  it("batch_edit rejects repeated noop edits at the loop threshold", async () => {
    await withTempFile("t.txt", "line one\n", async ({ cwd, path }) => {
      ...
      await expect(batchTool(harness).execute("batch_edit", { edits: [edit] })).rejects.toThrow(
        /E_NOOP_LOOP/,
      );
