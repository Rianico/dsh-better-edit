...
async function freshCwd(): Promise<{ cwd: string; restoreHome: () => void }> {
  const cwd = await mkdtemp(join(await getWritableTempRoot(), "dsh-hashline-test-"));
    ...
    batch_edit: {
      async execute(_callId: string, params: unknown) {
        ...
        for (const e of edits) {
          ...
          if (ep !== path)
            throw new Error(
              "[E_BAD_PAYLOAD] batch_edit shim: cross-file batches removed — use one edit call per file",
            );
        }
