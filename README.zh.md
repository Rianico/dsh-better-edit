...
**省 token。** 一次编辑调用只携带 `remove_from` / `remove_to`（两个 3 字符哈希）加替换文本——从不回显被替换的文本。`str_replace` 调用则必须逐字复现被替换的文本。在一个真实文件上的 12 次编辑会话中，这可以**减少 28% 的输出 token**（多行范围达 40%）——而且这些是*输出* token，按输入的约 5-6 倍计费。见[基准测试](#基准测试)。
...
| | hashline `edit` | `str_replace` | 行号编辑 |
| --- | :---: | :---: | :---: |
| 调用中回显被替换文本 | ✅ 不需要——两个哈希 | ❌ 逐字回显 | ✅ 不需要 |
| 对照模型所见内容校验 | ✅ 每一行 | ❌ 取第一个匹配 | ~ |
| 检测文件已过期 | ✅ 拒绝并回传新锚点 | ❌ 可能匹配到错误位置 | ~ |
| 上方编辑后锚点依然有效 | ✅ 内容寻址 | ✅ 基于内容 | ❌ 需要重读 |
| 连续编辑无需重读 | ✅ diff 提供新锚点 | ~ | ❌ |
| 文本重复时无歧义 | ✅ 边界锚点需校验 | ❌ 取第一个出现 | ~ |
| 错行编辑会悄悄落盘 | ❌ 不可能 | ✅ | ✅ |

> `~` = 偶尔/不稳定。行号编辑工具接受一个行范围，并在调用执行时应用到该偏移处的内容——成本低，但只要上方有任何内容移动就会失效。

## 基准测试

在同一份 103 行文件上、用相同的 12 组替换（8 个单行、4 个 3/6/10/15 行多行），以固定的 `js-tiktoken` `cl100k_base` 词表测量：

| 指标 | hashline | str_replace |
| ----------- | :---: | :---: |
| 被替换文本是否上线 | ✅ 从不 | ❌ 每次编辑都发 |
| 输出 token 节省（12 次编辑） | ✅ **28%** | ❌ 0% |
| 多行范围节省（3–15 行） | ✅ **25–46%** | ❌ 0% |
| 按 5 倍输出计价的实际成本 | ✅ **低约 1.4 倍** | ❌ 1× |
| 范围对照已提供状态校验 | ✅ 100% | ❌ 无 |
...
├── benchmark/           # 可复现的 hashline 与 str_replace token 基准测试
│   └── corpus/          # 固定的 103 行语料
├── test/                # 615 个测试（移植 + 回归）
