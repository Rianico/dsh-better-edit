...
清空或删除覆盖文件即可恢复该片段的编译内置默认指引与顺序：session-start 时渲染默认值，下次启动时重新写入该文件。

- **重置 = 删除文件，或清空内容并移除 front-matter 栅栏。** 没有栅栏、只有空白内容的文件表示“我要默认值”——渲染编译内置默认值，且文件会在下次启动时被重新写入（任何 preset 目录，随附或自定义均可）。
- **想故意留空？保留一个合法的栅栏。** 任何格式正确的 `---` 栅栏——即使是无键的 `---\n---\n`，即使正文为空——都是明确的意图信号：该文件是显式内容，绝不会被重置或重新写入。
- **损坏的栅栏 = 快速失败。** 无法解析的 `---` 栅栏（缺少闭合 `---`、`order` 非整数、未知键）会被拒绝：畸形文本绝不会注入上下文，改为渲染编译内置默认值，并记录一条指明文件与解析原因的警告；磁盘上的文件保持原样，供你修复。
- **随附 vs 自定义。** 随附 preset（`standard`、`code`、`minimal`、`cordis`）的文件会在下次启动时重新写入；删除的自定义 preset 覆盖文件保持缺失——缺失即无覆盖。删除整个 `<preset>/` 目录会在下次启动时重新写入全部四个片段文件（随附 preset）。
- **重置恢复的是当前 bundle 的默认值**——插件升级后会得到新的默认值。

重新写入只发生在启动时，绝不会在会话中途发生。
