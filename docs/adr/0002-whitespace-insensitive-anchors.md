...
- **Anchor-strip only, no fingerprint (chosen)** — the anchor absorbs ASCII whitespace; verification semantics for every _non-whitespace_ change stay identical to today. Linter-only workflows lose the rejection/re-read tax on whitespace churn; the ~2–7% token-level lint changes remain visible rejections, which is correct. Schema stays one value per line; no served-store change beyond the anchor derivation.
