...
- **Global `reserved=served∪retired` across _all_ sessions per path** (`getAnchorReservations(path)` scans `SELECT ... WHERE path=?`). ADR-0013 requires **per-`(session,path)` tombstone epoch** since last full `read`; global reservation blocks reuse for _all_ sessions until any session does a full read — prematurely cleared and violates per-session isolation. `file snapshots` stays global last-writer-wins, but `tombstone` must be per-session.
