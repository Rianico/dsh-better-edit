...
* **Lifecycle** — plugin-owned for `central`/custom, user-owned for `workspace`. Throttled janitor on `apply` + `agent/session-start` (>24h), `readdir runtime` → `mtime>30d` first, then LRU by `mtime` until `count<100 && sum<500MB` (`storeMaxAgeDays/storeMaxTotalBytes`). Row TTL inside live DBs: `SERVED 7d` (exists) + `undo 7d` (`undo_ttl_s: 604800 int -1=forever`) + `pruneMissing` batch 64 + `wal_checkpoint(TRUNCATE)` on close. No `pid` level — cross-process via `withBusyRetry` + `mtime` (live DB mtime stays hot). Migration probe copies legacy `<ws>/.dsh_better_edit/` once on first central open.
...
* **Schema:** yaml Zod `storeDir: z.enum(["workspace","central"]).or(z.string().refine(isAbsolute))`, `autoGitignore: z.boolean().default(false)`, `undo_ttl_s: z.number().int().min(-1).default(604800)`, `storeMaxAgeDays: z.number().int().min(1).default(30)`, `storeMaxTotalBytes: z.number().int().min(0).default(524288000)`. Env `DSH_BETTER_EDIT_AUTO_GITIGNORE` parsed `toLowerCase()==="true"` else false + warn on other strings.
...
* **Janitor:** `apply` `queueMicrotask(throttledJanitor)` + `agent/session-start` `if now-last>24h` → `readdir runtime`, `stat.mtimeMs`, never delete live `hash(workspaceCwd)` (`Set<hash(liveStores.keys())>`), evict `mtime>30d` then LRU. Row TTL: `DELETE FROM served WHERE updated_at < now-7d`, `DELETE FROM undo WHERE updated_at < now-undo_ttl_s` (skip -1), `pruneMissing()` batch 64, all `withStore`.
