...
export function isSupportedEncoding(enc: string): boolean {
  ...
  return (
    (SUPPORTED_ENCODINGS as readonly string[]).includes(norm) ||
    ["utf8", "utf8bom", "utf16le", "utf16be", "utf32le", "utf32be"].includes(norm)
  );
}
...
export function detectBom(bytes: Uint8Array): BomInfo | undefined {
  if (bytes.length >= 3 && bytes[0] === 0xef && bytes[1] === 0xbb && bytes[2] === 0xbf)
    return { encoding: "utf8bom", bomLen: 3, hasBOM: true };
  if (
    bytes.length >= 4 &&
    bytes[0] === 0xff &&
    bytes[1] === 0xfe &&
    ...
    return { encoding: "utf32le", bomLen: 4, hasBOM: true };
  if (
    bytes.length >= 4 &&
    bytes[0] === 0x00 &&
    bytes[1] === 0x00 &&
    ...
    return { encoding: "utf32be", bomLen: 4, hasBOM: true };
  if (bytes.length >= 2 && bytes[0] === 0xff && bytes[1] === 0xfe)
    return { encoding: "utf16le", bomLen: 2, hasBOM: true };
  if (bytes.length >= 2 && bytes[0] === 0xfe && bytes[1] === 0xff)
    return { encoding: "utf16be", bomLen: 2, hasBOM: true };
  return undefined;
}
 ...
 */
export async function detectWithChardet(
  bytes: Uint8Array,
  allowlist: string[],
  ...
  try {
    ...
    const analyse = (
      chardetLike as {
        analyse?: (b: Uint8Array | Buffer) => Array<{ name: string; confidence: number }>;
      }
    ).analyse;
