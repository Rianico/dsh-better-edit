...
export async function loadServedCanons(
  sessionKey: string,
  path: string,
): Promise<(string | null)[]> {
  const store = await loadServedStore();
...
function displacedHashes(
  ...
  return new Set(current.filter((hash): hash is string => hash !== null && !remaining.has(hash)));
}
...
function sweepAndRetire(
  ...
  for (const pos of servedPositions)
    if (!existingCards.has(pos)) {
      existingCards.add(pos);
      cardsChanged = true;
    }
  ...
  const afterSweep = existingEntries.filter(
    (e) => e.deathPos === null || !existingCards.has(e.deathPos),
  );
...
export async function recordServed(
  ...
  try {
    ...
    const isFullRead =
      ...
      rows.every((row, index) => row.position === index && row.hash === full.hashes[index]);
    withStore(() => {
