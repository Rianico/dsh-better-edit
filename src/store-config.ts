...
const StoreConfigSchema = z.object({
  ...
  supportedEncodings: z
    .array(z.string())
    .default(["gbk", "big5", "shift_jis", "euc-kr", "windows-1251", "iso-8859-1"]),
...
function fsAdapter(): Partial<StoreConfig> {
  ...
  try {
    ...
    if (parsed.autoGuessEncoding !== undefined) {
      ...
      if (b === undefined)
        console.warn(
          `dsh-better-edit: autoGuessEncoding "${parsed.autoGuessEncoding}" invalid — expected true|false`,
        );
    ...
    if (parsed.normalizeToUtf8 !== undefined) {
      ...
      if (b === undefined)
        console.warn(
          `dsh-better-edit: normalizeToUtf8 "${parsed.normalizeToUtf8}" invalid — expected true|false`,
        );
    ...
    if (parsed.supportedEncodings !== undefined) {
      ...
      if (arr === undefined)
        console.warn(`dsh-better-edit: supportedEncodings "${parsed.supportedEncodings}" invalid`);
...
function envAdapter(base: StoreConfig): Partial<StoreConfig> {
  ...
  if (envStoreDir !== undefined) {
    ...
    if (trimmed.length === 0) {
      console.warn(
        `dsh-better-edit: DSH_BETTER_EDIT_STORE_DIR empty — fallback to ${base.storeDir}`,
      );
    } else {
  ...
  if (envGuess !== undefined) {
    ...
    if (b === undefined)
      console.warn(
        `dsh-better-edit: DSH_BETTER_EDIT_AUTO_GUESS_ENCODING invalid — expected true|false`,
      );
    else out.autoGuessEncoding = b;
  }
  ...
  if (envNorm !== undefined) {
    ...
    if (b === undefined)
      console.warn(
        `dsh-better-edit: DSH_BETTER_EDIT_NORMALIZE_TO_UTF8 invalid — expected true|false`,
      );
    else out.normalizeToUtf8 = b;
  }
  ...
  if (envSup !== undefined) {
    ...
    if (arr === undefined)
      console.warn(`dsh-better-edit: DSH_BETTER_EDIT_SUPPORTED_ENCODINGS invalid`);
    else out.supportedEncodings = arr;
  }
...
function complementMissingFieldsSync(yamlPath: string, yamlPartial: Partial<StoreConfig>): void {
  ...
  if (yamlPartial.supportedEncodings === undefined)
    missingLines.push(
      "supportedEncodings: [gbk, big5, shift_jis, euc-kr, windows-1251, iso-8859-1]",
    );
  ...
  if (yamlPartial.storeMaxTotalBytes === undefined)
    missingLines.push("storeMaxTotalBytes: 524288000");
