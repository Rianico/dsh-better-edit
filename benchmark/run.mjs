...
function hashLine(content) {
  ...
  for (let k = 0; k < SPACE; k++) {
    const ch =
      ALPHABET[c % 62] + ALPHABET[Math.floor(c / 62) % 62] + ALPHABET[Math.floor(c / 3844) % 62];
...
const EDITS = [
  ...
  },
  {
    match: "qty <= 0",
    span: 1,
    replacement: ["    if (qty <= 0) throw new CartError('quantity must be positive');"],
    note: "single · guard",
  },
  {
    match: "total += item.unitPrice",
    span: 1,
    replacement: ["      total = this.round2(total + item.unitPrice * item.qty);"],
  ...
  },
  {
    ...
    span: 1,
    replacement: ["export function formatMoney(cents: number, withSymbol = true): string {"],
...
function render(r) {
  ...
  const sum = (es) =>
    es.reduce((s, e) => ({ hl: s.hl + e.hl, sr: s.sr + e.sr, omSeq: s.omSeq + e.omSeq }), {
      hl: 0,
      sr: 0,
      omSeq: 0,
    });
  const pct = (a, b) => (b === 0 ? "n/a" : `${Math.round(((b - a) / b) * 100)}%`);
  ...
  out.push("read traffic is identical for the tool arms and is excluded (it cancels).");
