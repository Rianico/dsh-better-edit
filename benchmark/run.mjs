...
const EDITS = [
  // single-line edits (8)
  { match: "TAX_RATE = 0.2", span: 1, replacement: ["export const TAX_RATE = 0.21;"], note: "single · constant" },
  { match: "An in-memory shopping cart for the demo API", span: 1, replacement: ["// An in-memory cart used by the demo API routes."], note: "single · comment" },
  { match: "this.items = new Map();", span: 1, replacement: ["    this.items = new Map<string, CartItem>();"], note: "single · assignment" },
  { match: "private round2", span: 1, replacement: ["  private round2(n: number, precision = 2): number {"], note: "single · signature" },
  { match: "qty <= 0", span: 1, replacement: ["    if (qty <= 0) throw new CartError('quantity must be positive');"], note: "single · guard" },
  { match: "total += item.unitPrice", span: 1, replacement: ["      total = this.round2(total + item.unitPrice * item.qty);"], note: "single · expression" },
  { match: "get total()", span: 1, replacement: ["  get totalWithTax(): number {"], note: "single · getter" },
  { match: "export function formatMoney", span: 1, replacement: ["export function formatMoney(cents: number, withSymbol = true): string {"], note: "single · export fn" },
  // multi-line edits (4)
  {
    match: "if (unitPrice < 0) {", span: 3,
  ...
  {
    match: "const existing = this.items.get", span: 6,
  ...
  {
    match: "checkout(): CheckoutReceipt", span: 15,
...
function run() {
  ...
  for (let i = 0; i < EDITS.length; i++) {
    ...
    const strReq = JSON.stringify({ path: PATH, old_string: rangeText, new_string: replacementText });
...
function render(r) {
  ...
  const sum = (es) => es.reduce((s, e) => ({ hl: s.hl + e.hl, sr: s.sr + e.sr }), { hl: 0, sr: 0 });
  const fmtPct = (hl, sr) => (sr === 0 ? "n/a" : `${Math.round(((sr - hl) / sr) * 100)}%`);
  const sep = "------------------|-------|----------|-------------|-------|-----";
  ...
  out.push(`edits    : ${r.edits.length} (${single.length} single-line, ${multi.length} multi-line)`);
  out.push(`tokenizer: ${TOKENIZER_NAME}`);
  out.push("");
  out.push("scenario            | lines | hashline | str_replace | saved |  %  |");
  out.push(sep);
  ...
  out.push("read traffic is identical for both tools and is excluded (it cancels).");
  out.push("these are the model's OUTPUT tokens (the edit call it emits), billed at ~5-6× input.");
  out.push(`at the 5× output rate, hashline costs ${Math.round((t.sr / t.hl) * 10) / 10}× less than str_replace on effective cost.`);
  out.push("savings scale with the replaced text: ~0% for short single lines, 25-46% for multi-line ranges.");
  out.push(
